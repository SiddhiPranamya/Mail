    ...
    .then(emails => {
      ...
      for (let email of emails) {
        if (mailbox == 'inbox' && email.archived) {
          ...
          mailDiv.setAttribute("class", "border border-secondary mt-3");
          ...
          mailDiv.innerHTML += "Subject: " + email.subject + "<br/>";
          mailDiv.innerHTML += email.timestamp + "<br/>";
          document.querySelector('#emails-view').appendChild(mailDiv);
          mailDiv.addEventListener('click', () => load_email(email));
...
function load_email(email) {
  ...
  .then(response => response.json())
  .then(email => {
      document.querySelector('#show-email').innerHTML = `<h3>${email.subject}</h3>`;
      
      const mailDiv = document.createElement('div');
      mailDiv.setAttribute("class", "border mt-2");
      mailDiv.innerHTML += "From: " + email.sender + "<br/>";
      mailDiv.innerHTML += "Recipients: ";
      for (let recipient of email.recipients) {
        mailDiv.innerHTML += recipient;
      }
      mailDiv.innerHTML += "<br/>";
      mailDiv.innerHTML += "Sent on " + email.timestamp + "<br/>";

      const bodyDiv = document.createElement('div');
      bodyDiv.setAttribute("class", "border ");
      bodyDiv.innerHTML += email.body + "<br/>";
      document.querySelector('#show-email').appendChild(mailDiv);
      document.querySelector('#show-email').appendChild(bodyDiv);

      const replyButton = document.createElement('button'); 
      replyButton.setAttribute("class", "btn btn-primary");
      replyButton.textContent = "Reply";
      document.querySelector('#show-email').appendChild(replyButton);
